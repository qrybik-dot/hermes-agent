...
def create_job_with_scheduler_registration(*, _create_job_fn=None, **kwargs) -> dict:
    """Persist one job and register its first trigger with the active provider.

    A reconciled gateway-message replay returns the already persisted job and
    must not register a duplicate external trigger. ``_create_job_fn`` is an
    internal injection seam used by the model-tool replay/fault tests.
    """
    from cron.jobs import create_job as default_create_job
    from cron.scheduler_provider import resolve_cron_scheduler

    create_fn = _create_job_fn or default_create_job
    job = create_fn(**kwargs)
    if job.get("_create_reconciled"):
        return job
    try:
