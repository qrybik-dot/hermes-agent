...
def skills_facts_operational_context() -> str:
    return (
        ...
        "Не выдумывай команды установки. Если проверка недоступна, верни PARTIAL: "
        "коротко отдели проверенные факты от неизвестного и назови один следующий безопасный шаг. "
        "BLOCKED используй только когда без решения пользователя или недостающего обязательного входа "
        "нельзя выполнить ни одного безопасного действия."
    )
...
def adaptive_planning_operational_context(policy: AdaptivePlanningPolicy) -> str:
    ...
    common = (
        "Адаптивное планирование: используй skill plan, если он доступен; иначе составь тот же план "
        "встроенно и продолжай без блокировки. Отсутствие вспомогательного skill не является причиной "
        "останавливать задачу. Если пользователь запросил только план, не выполняй изменения. Если вместе "
        "с планом запросил реализацию, не останавливайся после плана. Не создавай отдельный plan-файл без явного запроса. "
