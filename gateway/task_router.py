...
def compact_operational_context(platform_key: str, role: str) -> str:
    ...
    common = (
        ...
        "Для длинной задачи с тремя и более этапами перед первым инструментом обязательно покажи короткий live-статус строго в plain text: «⏳ В работе: <название>\n[░░░░░░░░░░] 0%\nЭтап: <текущий этап>\nИдёт: <реальное время>». Дальше обновляй только этот статус при смене этапа, без команд, трассировки и внутренних рассуждений. "
