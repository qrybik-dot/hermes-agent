...
class TestSystemdUnitIsCurrent:
    def test_externally_managed_runtime_unit_is_not_auto_refreshed(
        self, tmp_path, monkeypatch,
    ):
        """The release manager, not the Hermes CLI, owns managed units.

        A managed immutable-runtime unit intentionally differs from the unit
        generated by ``hermes gateway install`` (shared venv, release symlink,
        resource guards).  Treating that difference as staleness makes
        ``hermes gateway restart`` overwrite the production control plane.
        """
        from hermes_cli import gateway as gw

        installed = """[Service]
Environment="HERMES_RUNTIME_ROOT=/home/hermes/hermes-runtime/current"
ExecStart=/home/hermes/hermes-runtime/shared/venv/bin/python -m hermes_cli.main gateway run
WorkingDirectory=/home/hermes/hermes-runtime/current
"""
        expected = """[Service]
ExecStart=/home/hermes/hermes-runtime/current/.venv/bin/python -m hermes_cli.main gateway run
...
"""
        ...
        assert gw.systemd_unit_is_current(system=True) is True

    def test_commented_managed_marker_does_not_bypass_stale_check(
        self, tmp_path, monkeypatch,
    ):
        from hermes_cli import gateway as gw
        ...
        installed = """[Service]
# Environment="HERMES_RUNTIME_ROOT=/home/hermes/hermes-runtime/current"
ExecStart=/old/python -m hermes_cli.main gateway run
"""
        expected = """[Service]
ExecStart=/new/python -m hermes_cli.main gateway run
"""
        unit_file = tmp_path / "hermes-gateway.service"
        unit_file.write_text(installed)
